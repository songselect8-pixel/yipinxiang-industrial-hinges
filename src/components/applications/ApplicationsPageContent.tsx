      ...
      <div className="shell">
        ...
        <div className="application-overview-grid">
          {applicationOverview.map((item, index) => (
            <a className={`application-overview-card${index < 2 ? " application-overview-card-primary" : " application-overview-card-secondary"}`} href={item.href} key={item.id}>
          ...
          {applicationGroups.map((group) => (
            <article key={group.id}>
              ...
              <nav aria-label={`Product navigation for ${group.shortTitle}`}>
                {group.productIds.map((id) => <a href={productHref(id)} key={id}>{getProduct(id)!.name}<Arrow /></a>)}
              </nav>
              <a href={group.filterHref} className="text-link">Explore relevant hinge families <Arrow /></a>
            </article>
          ))}
        </div>
        <div className="application-navigation-note"><p>Need to review another hinge structure?</p><a href="/products" className="button button-outline">Explore all hinge types <Arrow /></a></div>
      </div>
    </section>
  );
}
  ...
  return (
    <>
      <section className="application-page-hero" aria-labelledby="application-page-title">
        <div className="shell">
          <nav className="application-breadcrumb" aria-label="Breadcrumb"><ol><li><a href="/">Home</a></li><li aria-hidden="true">/</li><li aria-current="page">Applications</li></ol></nav>
          <div className="application-hero-layout">
            <div className="application-hero-copy">
              ...
              <div className="application-hero-actions"><a href="#application-overview" className="button button-primary">Explore Applications <Arrow /></a><ApplicationInquiryLink product="custom" application="" className="button button-outline">Ask for Selection Help</ApplicationInquiryLink></div>
            </div>
